...
At the end of every response where code changes are made, suggest a git commit message following this format:

**Commit message rules:**
- Write in English
- Start with an emoji prefix based on the change type
...
✨ Add user authentication with Clerk integration
```
## 開発サーバー起動ポリシー
⚠️ **開発サーバーの自動起動禁止**:
- `npm run dev`、`yarn dev`、`pnpm dev` などの開発サーバー起動コマンドを**勝手に実行しないこと**
- 開発サーバーの起動はユーザーが手動で行う
- コード修正後に「開発サーバーで確認してください」などの指示のみを出すこと

## セキュリティポリシー（最重要）
⚠️ **絶対に以下のファイルを読み取らないこと**:
- `.env`
- `.env.local`
- `.env.production`
- `.env.development`
...
これらのファイルにはシークレットキー、パスワード、APIキーなどの機密情報が含まれています。
Read ツールでこれらのファイルを開くことは**厳禁**です。

環境変数の設定が必要な場合は、ユーザーに直接指示を出してください。
